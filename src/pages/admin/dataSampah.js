...
async function addDataToFirebase(jenisSampah, harga, poin, tanggalAdd,
) {
    try {
...
export default function DataSampah() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const added = await addDataToFirebase(jenisSampah, harga, poin, tanggalAdd,
        );
        if (added) {
            setjenisSampah("");
            setharga("");
    ...
    const selectedItem = dataSampah.find(item => item.jenisSampah === selectedNama);
        ...
        <>
            <div className="jurusan">
                <div className="text-center mb-5">
                    <h3 className="mb-4">DATA SAMPAH ANORGANIK</h3>
                    <select
                    ...
                    >
                        ...
                        {dataSampah.map((item, index) => (
                            <option key={index} value={item.jenisSampah}>{item.jenisSampah}</option>
                        ))}
                    </select>
                ...
                {!details ? (
                    <section>
                        <form onSubmit={handleSubmit} method="post" action="">
                            ...
                            <select onChange={(e) => setjenisSampah(e.target.value)} required>
                            ...
                            <p>Harga Jual</p>
                            <input type="number" onChange={(e) => setharga(e.target.value)} required />

                            <p>Poin Yang Didapatkan</p>
                            <input type="number" onChange={(e) => setpoin(e.target.value)} required />
                    ...
                    <section className="detail ps-5 pb-5">
                        <span>
                            <p>Jenis Sampah Anorganik</p>
                            <select name="jenisSampah"
                                onChange={handleInputChange}
                                value={formData.jenisSampah || ''} required>
                                <option selected disabled>Pilih</option>
                                <option value="Plastik">Plastik</option>
                                <option value="Kaca">Kaca</option>
                                <option value="Besi">Besi</option>
