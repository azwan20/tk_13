...
async function addDataToFirebase(namaUser, jenisKelamin, noHp, type, tanggalAdd,
) {
    try {
        const docRefSurat = await addDoc(collection(db, "user"), {
            namaUser: namaUser,
...
export default function DataUser() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const added = await addDataToFirebase(namaUser, jenisKelamin, noHp, type, tanggalAdd,
        );
        if (added) {
            setNamaUser("");
            setJenisKelamin("");
    ...
    const selectedItem = dataUser.find(item => item.namaUser === selectedNama);
    ...
    const handleSaveClick = async () => {
        if (formData.id) {
            const docRef = doc(db, "user", formData.id);
        ...
        <>
            <div className="jurusan">
                <div className="text-center mb-5">
                    <h3 className="mb-4">DATA USER</h3>
                    <select
                    ...
                    >
                        ...
                        {dataUser.map((item, index) => (
                            <option key={index} value={item.c}>{item.namaUser}</option>
                        ))}
                    </select>
                </div>
                {!details ? (
                    <section>
                        <form onSubmit={handleSubmit} method="post" action="">
                            <p>Nama User:</p>
                            <input type="text" onChange={(e) => setNamaUser(e.target.value)} required />

                            <p>Jenis Kelamin</p>
                            <select onChange={(e) => setJenisKelamin(e.target.value)} required>
                    ...
                    <section className="detail ps-5 pb-5">
                        ...
                        <span>
                            ...
                            <select name="jenisKelamin"
                                onChange={handleInputChange}
                                value={formData.jenisKelamin || ''} required>
                                <option selected disabled>Pilih</option>
                                <option value="Laki-laki">Laki-laki</option>
                                <option value="Perempuan">Perempuan</option>
                            </select>
